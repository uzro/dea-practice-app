...
export default function QuickExam() {
  ...
  const [timeLimit] = useState(40 * 60) // 40分钟 = 2400秒
  ...
  useEffect(() => {
    if (currentExam && !examResult) {
      const timer = setInterval(() => {
        ...
        // 时间警告（5分钟时提醒）
        if (elapsed === timeLimit - 300 && timeLimit - elapsed === 300) {
          alert('注意：考试时间剩余 5 分钟，请尽快完成答题！')
        }
      }, 1000)
  ...
  const initializeExam = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/exam?type=quick')
      ...
      startExam('quick', data.questions)
    } catch (error) {
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white rounded-lg shadow-md p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">
              快速考试完成 - {examResult.score >= 70 ? '通过' : '未通过'}
            </h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900">快速考试</h1>
          </div>
          
          <div className="flex items-center space-x-4">
            <div className="text-lg font-semibold text-gray-900">
              剩余时间: <span className={`${
                timeLimit - timeElapsed <= 300 ? 'text-red-600' : 'text-gray-900'
